...
if ( ! wp_next_scheduled( 'fetch_catalog_event' ) ) {
  wp_schedule_event( time(), 'daily', 'fetch_catalog_event' );
}
add_action( 'fetch_catalog_event', array( PadelPoint\Job::class, 'run_weekly_imports' ) );
...
add_action( 'woocommerce_order_status_completed', array( PadelPoint\Job::class, 'process_order' ) );
...
add_action( 'admin_action_' . PadelPoint\Constants::ACTION_SLUG_SYNCRONIZE_PRODUCTS, $callback );
